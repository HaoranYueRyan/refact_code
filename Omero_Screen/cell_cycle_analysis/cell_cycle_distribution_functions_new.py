...
def fun_CellCycle (data,
    ...
    for cell_line in data["cell_line"].unique() :
        ...
        tmp_data = data.copy().loc[data["cell_line"] == cell_line]
        ...
        1.4) Determining the three thresholds (low, mid, high).
        ...
        """1.4"""
        DAPI_low_threshold = DAPI_hist.iloc[(DAPI_peaks_selection[0] - DAPI_peaks_distance_half)]["interval"].left
        ...
        tmp_data["cell_cycle"] = np.where(tmp_data["cell_cycle_detailed"].isin(["G2", "M"]), "G2/M",
                                          np.where(tmp_data["cell_cycle_detailed"].isin(["Early S", "Late S"]), "S", tmp_data["cell_cycle_detailed"]))
